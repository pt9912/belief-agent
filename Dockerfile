...
COPY adapters/outbound/audit-file/build.gradle.kts ./adapters/outbound/audit-file/build.gradle.kts
...
COPY adapters/outbound/approval-remote-ui/build.gradle.kts ./adapters/outbound/approval-remote-ui/build.gradle.kts
...
COPY adapters/outbound/llm-action-langchain4j/build.gradle.kts ./adapters/outbound/llm-action-langchain4j/build.gradle.kts
COPY adapters/outbound/llm-action-koog/build.gradle.kts ./adapters/outbound/llm-action-koog/build.gradle.kts
...
RUN gradle --no-daemon --console=plain --max-workers=1 \
    ...
    :adapters:outbound:llm-action-langchain4j:test \
    :adapters:outbound:llm-action-koog:test \
    ...
    :adapters:outbound:audit-file:test \
...
RUN gradle --no-daemon --console=plain \
    ...
    :adapters:outbound:audit-memory:koverLog :adapters:outbound:audit-file:koverLog :adapters:outbound:approval-fake:koverLog \
    :adapters:outbound:approval-local:koverLog \
    :adapters:outbound:approval-remote-ui:koverLog \
    ...
    :adapters:outbound:llm-action-langchain4j:koverLog \
    :adapters:outbound:llm-action-koog:koverLog \
...
RUN gradle --no-daemon --console=plain --max-workers=1 \
    ...
    :adapters:outbound:audit-memory:koverVerify :adapters:outbound:audit-file:koverVerify :adapters:outbound:approval-fake:koverVerify \
    :adapters:outbound:approval-local:koverVerify \
    :adapters:outbound:approval-remote-ui:koverVerify \
    ...
    :adapters:outbound:llm-action-langchain4j:koverVerify \
    :adapters:outbound:llm-action-koog:koverVerify \
...
# --- cli-demo-approval-remote-ui: Remote/UI-Kanal, EOF bleibt fail-closed ---
FROM build AS cli-demo-approval-remote-ui
RUN gradle --no-daemon --console=plain :adapters:inbound:cli:run --args='eskaliert approval=remote-ui'
