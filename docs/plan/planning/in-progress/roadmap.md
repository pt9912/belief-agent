...
`done/` abgeschlossen
([`slice-038-approval-kanalwahl`](../done/slice-038-approval-kanalwahl.md)).

`slice-037` ist als CLI-Binding fuer den lokalen Approval-Adapter auf `done/`
abgeschlossen
([`slice-037-cli-approval-binding`](../done/slice-037-cli-approval-binding.md)).
...
`slice-035` ist als Approval-Kontextvertrags-Slice auf `done/` abgeschlossen
...
Approval-Audit-Persistenz (`slice-040`, done), persistenter AuditPort-Adapter
...
Hypothesen-Provider (`slice-048`), Code-Agent BeobachtungsAuswahl-Composition
(`slice-049`), CLI-BeobachtungsAuswahl-Default-Binding (`slice-050`), echte
...
|---|---|---|---|
| M1 — Belief-Kern lauffähig | welle-01 (`slice-001`..`slice-004`) | ungültiger Belief State wird nachweislich zurückgewiesen ([`LH-FA-BEL-004`](../../../../spec/lastenheft.md#lh-fa-bel-004--zurückweisung-ungültiger-belief-states)) **und** deterministisches Bayes-Update grün (`make test`) | **erreicht (2026-07-04)** |
...
| `welle-03-aktionen-gates` | 2026-07-05 | Sicherheitsfunktion ([`MR-003`](../../../../harness/conventions.md#mr-003--repo-klasse-safetycontrol)): Wirkungsklassen + Konfidenz-Gate + menschliche Freigabe; 102 Tests, 97,65 % Coverage (domain); [Ergebnisse](../done/welle-03-aktionen-gates-results.md). 2 Code-Reviews (7 Safety-Befunde fail-closed gefixt), [`ADR-0005`](../../adr/0005-konfidenz-gate-schwellwerte.md) Accepted. |
| `welle-04-voi-eskalation` | 2026-07-06 | Entscheidungszyklus (`ARC-09`: sammeln \| handeln \| eskalieren); VoI-Selektor + Eskalation + Budget + `voi-fake`; 151 Tests, 98,17 %/98,73 % Coverage (domain/app); [Ergebnisse](../done/welle-04-voi-eskalation-results.md). Sequentielles + Ketten-Review (10 Befunde), [`ADR-0007`](../../adr/0007-eskalations-schwelle.md) Accepted. |
...
| 2026-07-04 | Welle-01 gestartet (Status `in-progress`); Slices `slice-001`..`slice-004` in `open/` angelegt | Start-Trigger erfüllt: [`ADR-0001`](../../adr/0001-hexagonal-llm-port.md) & [`ADR-0002`](../../adr/0002-implementierungssprache-jvm-java.md) Accepted |
...
| 2026-07-04 | welle-02-evidenz-audit aufgesetzt (`slice-005`..`slice-008` in `open/`); d-check-Module erweitert ([`MR-006`](../../../../harness/conventions.md#mr-006--d-check-modul-auswahl-erweitert-v0371)) + `version.md` | welle-01 done → nächste Welle |
| 2026-07-05 | `slice-006` geliefert (Dedup korrelierter Beobachtungen, [`LH-FA-OBS-004`](../../../../spec/lastenheft.md#lh-fa-obs-004--deduplizierung-korrelierter-beobachtungen)); Resume-Punkt → `slice-007` | `make gates` grün (46 Tests, 96,81 % Coverage); DoD erfüllt, liegt in `in-progress/` bis Welle-Closure |
| 2026-07-05 | `slice-007` geliefert (Ereignisprotokoll + Rekonstruktion, [`LH-FA-AUD-001`](../../../../spec/lastenheft.md#lh-fa-aud-001--unveränderliches-ereignisprotokoll)/`002`/`003`); Audit-Port nach `slice-008` verschoben (Weg C); Resume-Punkt → `slice-008` | `make gates` grün (59 Tests, 97,37 % Coverage); Audit-Port ist anwendungsweiter Port → application-Schicht (`architecture.md` §2), nicht Domäne; slice-007 bleibt reiner domain-Slice |
...
| 2026-07-05 | `slice-008` (Fundament) geliefert; a-check v0.10.0 → **v0.11.0** (Multi-Modul-KMP-Resolution, [`MR-005`](../../../../harness/conventions.md#mr-005--a-check-arch-gate-v0110-hexslice-rollen-multi-modul)); Resume-Punkt → `slice-009` | v0.10.0 konnte Multi-Modul nicht durchsetzen (Guard-Reject bzw. falsch-grün, negativ-getestet); Fix-Prompt an a-check → v0.11.0 löst datei-mengen-bewusst auf, echt durchsetzend; kein Carveout |
...
| 2026-07-05 | `slice-010` geliefert (Beobachtungs-Port + Quelle-/Audit-Adapter + E2E); **welle-02-Closure-Trigger erfüllt** | `make gates` grün (71 Tests); E2E `Quelle→Update→Protokoll→Persistenz→Rekonstruktion` demonstriert die Welle-Ziele ([`LH-FA-OBS-001`](../../../../spec/lastenheft.md#lh-fa-obs-001--heterogene-beobachtungsquellen)/`002`, [`LH-FA-AUD-002`](../../../../spec/lastenheft.md#lh-fa-aud-002--rekonstruierbarkeit-des-belief-states)) |
...
| 2026-07-05 | welle-03 aktiviert; `slice-011` geliefert (Domäne Aktion + 4 Wirkungsklassen + Erfolgs-P + Evidenz-Ref, [`LH-FA-ACT-001`](../../../../spec/lastenheft.md#lh-fa-act-001--vier-wirkungsklassen)..`004`); Resume-Punkt → `slice-012` | `make gates` grün (78 Tests, 97,71 % Coverage); Ruhe-Marker → welle-03 aktiv (slice-011 in `in-progress/`) |
| 2026-07-05 | `slice-012` geliefert (Konfidenz-Gate-Regel + [`ADR-0005`](../../adr/0005-konfidenz-gate-schwellwerte.md) Schwellwerte, [`LH-FA-POL-001`](../../../../spec/lastenheft.md#lh-fa-pol-001--konfidenz-gate-vor-jeder-aktion)/`002`/`003`/`005`/`007`); Resume-Punkt → `slice-013` | `make gates` grün (88 Tests, 98,1 % Coverage); Sicherheitskern ([`MR-003`](../../../../harness/conventions.md#mr-003--repo-klasse-safetycontrol)), fail-safe (Resthypothese-Sperre schlägt hohe Erfolgs-P) negativ-getestet |
| 2026-07-05 | Code-Review slice-012: 2 Safety-Inversionen in `GateSchwellen` gefixt (fail-closed Monotonie + Sperr-Schwelle `< 1`); [`ADR-0005`](../../adr/0005-konfidenz-gate-schwellwerte.md) → Accepted | Multi-Agent-Review fand config-erreichbare unsichere Freigabe-Pfade; 5 Tests ergänzt (93 gesamt) |
| 2026-07-05 | `slice-013` geliefert (aktion-gaten: nicht-umgehbares Gate + Human-Approval-Port, [`LH-FA-POL-004`](../../../../spec/lastenheft.md#lh-fa-pol-004--menschliche-freigabe-für-extern-wirksame-aktionen)/`006`); **welle-03-Closure-Trigger erfüllt** | `make gates` grün (101 Tests); Gate-Kette E2E (extern-wirksam nur mit Freigabe frei, sonst Eskalation) |
...
| 2026-07-05 | Coverage-Gate auf `application` + Adapter erweitert ([`ADR-0006`](../../adr/0006-coverage-gate-scope.md), per-Modul kover, kein zentraler Block); Sicherheitskern `AktionGaten` jetzt gate-erzwungen | `make gates` grün; Ist-Coverage application + Adapter 100 %, domain 97,65 % → 90 %-Floor |
...
| 2026-07-05 | **Tagesabschluss** — Resume-Punkt: welle-04 starten via `slice-014` | 3 Wellen done + Coverage-Scope ([`ADR-0006`](../../adr/0006-coverage-gate-scope.md)) + welle-04 aufgesetzt; alle Gates grün, Working Tree sauber |
| 2026-07-06 | **welle-04 gestartet**; `slice-014` `open → in-progress`, Ruhe-Marker aufgelöst; `slice-014` geliefert (VoI-Selektor + `VoiKandidat`, neue Sub-Area `hexagon:domain/voi`, [`LH-FA-VOI-002`](../../../../spec/lastenheft.md#lh-fa-voi-002--diskriminierung-der-zwei-wahrscheinlichsten-hypothesen)/`003`/`004`); Resume-Punkt → `slice-015` | Start-Trigger „welle-03 done" erfüllt; reine Domänen-Regel als kleiner Einstieg; `make gates` grün (doc-check/build/test/coverage-gate/arch-check 0 Befunde, domain 97,81 %), 14 neue Tests |
| 2026-07-06 | Planungs-Konvention: **keine eigenständigen Wellen-Dateien** mehr ([`MR-009`](../../../../harness/conventions.md#mr-009--keine-eigenständigen-wellen-plan-dateien-welletemplatemd-nicht-adoptiert)); `welle-01..04-*.md` + `welle.template.md` entfernt, Wellen nur als Roadmap-Eintrag + `done/…-results.md` | Referenz-Projekt-Modell (`lab/example`); 23 Verweise umgebogen; `make doc-check` grün |
| 2026-07-06 | `slice-015` `open → in-progress` **geliefert** (Eskalation-Zustand + Bedingung + Budget, neue Sub-Area `hexagon:domain/eskalation`, [`LH-FA-ESK-001`](../../../../spec/lastenheft.md#lh-fa-esk-001--eskalationsbedingung)..`004`); Resume-Punkt → `slice-016` | Domänen-Bausteine der Welle-04-Eskalation; θ_esc an θ_rehyp gekoppelt; `make gates` grün (domain 98,13 %), 21 neue Tests |
...
| 2026-07-06 | **Sequentielles Code-Review** slice-014/015 (Fail-safe, rollierend): 5 Befunde gefixt — Eskalations-Schwelle spec-konform (θ_esc **0,5→0,30**, `>`→`≥`, [`ADR-0007`](../../adr/0007-eskalations-schwelle.md), entkoppelt von Gate-Sperre); `schwelle` fail-closed; `Eskalationsgrund` trägt `GateEntscheidung` statt String; `VoiSelektor` Kreuz-Multiplikation statt Float-Division | F1 war un-ADR'te Safety-Schwelle + Unter-Eskalation im Band [0,30…0,50]; Reviews der Sicherheitsfunktion früh; `make gates` grün; offen: `STANDARD_SCHWELLWERT`-Reconciliation |
| 2026-07-06 | `slice-016` `open → in-progress` **geliefert** (`beobachtung-waehlen`: `BeobachtungsAuswahlPort` + Use-Case `BeobachtungWaehlen` + neues Adapter-Modul `adapters:outbound:voi-fake`, [`LH-FA-VOI-002`](../../../../spec/lastenheft.md#lh-fa-voi-002--diskriminierung-der-zwei-wahrscheinlichsten-hypothesen)); Resume-Punkt → `slice-017` | Erstes application-Slice der Welle; **Multi-Modul-/Build-Risiko isoliert & retired** (7 Module, arch-check grün); `make gates` grün (application/voi-fake 100 %), 4 neue Tests |
| 2026-07-06 | `slice-017` `open → in-progress` **geliefert** (`entscheidungszyklus`, `ARC-09`: `Entscheidungszyklus` + `Zyklusergebnis` verdrahten VoI + Belief-Update + Gate + Eskalation zu sammeln/handeln/eskalieren, [`LH-FA-VOI-001`](../../../../spec/lastenheft.md#lh-fa-voi-001--information-vor-handlung-bei-unsicherheit)); **Welle-04-Closure-Trigger erfüllt** | Letztes Welle-Slice; E2E gegen Fake-Ports (6 Fälle, beide Eskalations-Auslöser, budget-garantierte Terminierung [`LH-QA-02`](../../../../spec/lastenheft.md#lh-qa-02--konservatives-standardverhalten-fail-safe)); Aktionsfreigabe→GateEntscheidung-Rück-Mapping (Domäne kennt application nicht); `make gates` grün (application 100 %) |
| 2026-07-06 | **Ketten-Review** slice-016/017 (VoI + Eskalation + Zyklus): 5 Befunde gefixt — fehlende Freigabe wird jetzt **eskaliert statt still abgelehnt** (F1, [`LH-FA-POL-004`](../../../../spec/lastenheft.md#lh-fa-pol-004--menschliche-freigabe-für-extern-wirksame-aktionen)); Kandidaten-**Konsumption** gg. Scheingewissheit (F4a, [`LH-FA-OBS-004`](../../../../spec/lastenheft.md#lh-fa-obs-004--deduplizierung-korrelierter-beobachtungen)); `Eskalationsgrund.GateEskalation` (F2); Approval-Pfad-Test (F3); `ARC-09`-Diagramm reconcilt (F5) | Ketten-Sicht fand Kompositions-Fehler, die Einzel-Slices verbargen (welle-03-Lehre bestätigt); `make gates` grün; offen: belief-**abhängige** Kandidaten-Generierung (F4b) = welle-05 |
...
| 2026-07-06 | `slice-018` (Schwellwert-Reconciliation) **erledigt** → `done/`: Schwellen spec-konform verschärft (θ_other_block 0,5→0,10, θ_repo 0,7→0,80, θ_extern 0,9→0,95, θ_rehyp 0,5→0,30); [`ADR-0008`](../../adr/0008-schwellwerte-spec-konform.md) **supersedes** [`ADR-0005`](../../adr/0005-konfidenz-gate-schwellwerte.md); Resume → welle-05 | Source Precedence: Spec (Rang 2) sticht ADR (Rang 4); [ADR-0005](../../adr/0005-konfidenz-gate-schwellwerte.md) hatte Safety-Schwellen gelockert; nur 3 Grenzfall-Tests betroffen; `make gates` grün |
...
| 2026-07-07 | `slice-020` **in `done/` abgeschlossen**: `BeobachtungsAuswahlPort` belief-aware, `voi-fake` Top-2-konfigurierbar, Beispiele/Doku angepasst | `make gates` grün; F4b geschlossen ohne Erweiterung der [`LH-FA-LLM-002`](../../../../spec/lastenheft.md#lh-fa-llm-002--abgegrenzte-modell-aufgaben)-Modellaufgaben |
...
| 2026-07-08 | `slice-036` **in `done/` abgeschlossen**: lokaler Human-Approval-Adapter mit Nonce, Identitaet, Kontext-Digest und Einmaligkeit | `make gates` gruen; Code-/Safety-Review und Verification ohne Findings/DoD-Verletzung; CLI bleibt bewusst auf Fake/konfiguriertem Adapter, Binding folgt in `slice-037`. |
...
| 2026-07-08 | `slice-037` **in `done/` abgeschlossen**: CLI-Binding fuer lokalen Approval-Adapter | `make gates` gruen; `make cli-demo` und `make cli-demo-scenarios` gruen; Code-/Safety-Review und Verification ohne Findings/DoD-Verletzung. `approval=local` ist explizit, Fake-Szenario-Defaults bleiben erhalten. |
...
| 2026-07-08 | `slice-040` **in `done/` abgeschlossen**: Approval-Audit-Persistenz | `make gates` gruen; `make cli-demo-approval-remote-ui` gruen und fail-closed ohne Antwort; Code-/Safety-Review ohne Findings und Verification ohne DoD-Verletzung. `HumanApprovalPort` liefert `ApprovalErgebnis` mit Snapshot, `AktionGaten` schreibt `ApprovalAngefragt` plus Ergebnisereignis ueber `AuditPort`, und Audit-Ausfall bleibt fail-closed. |
...
| 2026-07-08 | `slice-049` in `open/` geplant: Code-Agent BeobachtungsAuswahl-Composition | Folgeslice zu `slice-045`: `example/code-agent` bindet den echten `BeobachtungsAuswahlPort`-Adapter ueber einen reproduzierbaren Kandidatenkatalog. Produktives CLI-Default-Binding, Adapter-zu-Adapter-Kopplung und Aenderungen an VoI-/Gate-/Executor-Logik bleiben getrennt. |
| 2026-07-08 | `slice-050` in `open/` geplant: CLI-BeobachtungsAuswahl-Default-Binding | Folgeslice zu `slice-045` und idealerweise `slice-049`: `adapters/inbound/cli` bindet den echten `BeobachtungsAuswahlPort`-Adapter als bewusst konfigurierbaren Default. Fake-/Szenario-Modus, Fehlerdiagnosen und VoI-/Gate-/Executor-Policy bleiben explizit getrennt. |
| 2026-07-09 | `slice-041` Review-Kette abgeschlossen (Erst-Lauf + Rerun + Rerun2 + **unabhängiger Frischkontext-Lauf**), Findings in Plan-§9 aufgelöst | Der unabhängige Lauf fand 3 MEDIUM (Write-Konsumenten-Asymmetrie, Restart-vs-Teil-Write, Coverage-Gate-/ADR-Referenz) + 2 INFO, die die Same-Context-Kette („0 Blocker") übersah — code-verifiziert. Vier deferred Folgeslices (`slice-051`..`054`) benannt, statt still zu verschieben (Modul 5). |
| 2026-07-09 | `slice-051` in `open/` geplant: Write-Konsumenten Audit-Schreibfehler fail-closed | Deferred aus `slice-041` §9 (IDR-1/IPR-1): `AktionsVorschlagen`/`KonfidenzExternalisieren`/code-agent verschlucken bzw. propagieren einen Audit-Schreibfehler uncaught. Fällig **vor** Bindung eines werfenden Adapters in den Vorschlags-/Konfidenzpfad. |
